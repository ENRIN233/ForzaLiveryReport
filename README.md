...
| 🔄 **重复涂装检测** | 双轨算法：车型+缩略图大小 & 车型+标题，并查集合并重叠组 |
...
同一车型上相同标题或相同缩略图的涂装会被识别为一组重复，每组分配独立颜色高亮。

## 工作原理

```
Data_Car.str  ──→  apply_all.js  ──→  注入 CAR_NAME_MAP (651 车型)
                                           +
...
- `Data_Car.str` — ForzaTech 二进制字符串表，VALUES/KEYS 双段哈希对齐解析
- `CAR_NAME_MAP` — 651 条目，覆盖 FH6 全部可驾驶车辆
- 报告为单文件 HTML，体积约 15MB（取决于涂装数量），可直接分享
