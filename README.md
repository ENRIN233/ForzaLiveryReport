...
| 🔄 **重复涂装检测** | 双路径并集：路径 A 缩略图大小固定锚点聚类（容差 0.5%）+ 路径 B 同车型同作者标题/描述匹配，并查集合并 |
...
路径 B（文本）  ：同车型 + 同作者 → 桶内标题必须相同；描述非空时也须相同
              ↓
    并查集取并集 → 连通分量 ≥2 项 = 最终重复组
```

同一车型上缩略图一致或标题/描述相同的涂装会被识别为一组重复，每组分配独立颜色高亮。
