...
let m=false;
if(existingCount!==dsCount||!/'\d{2}/.test(la.slice(la.indexOf('const CAR_NAME_MAP = {'),la.indexOf('};',la.indexOf('const CAR_NAME_MAP = {'))+2))||!la.includes('Porsche 911 Rallye')){
  ...
  const lines=[];
  Object.entries(cd).sort((a,b)=>parseInt(a[0])-parseInt(b[0])).forEach(([code,v])=>{const b=mfr(v.ms,parseInt(code));let n=b&&!v.d.toLowerCase().startsWith(b.toLowerCase())?b+' '+v.d:v.d;const ym=v.ms.match(/'(\d{2})$/);if(ym)n+=' '+ym[0];lines.push('  '+code+': "'+n.replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'",')});
